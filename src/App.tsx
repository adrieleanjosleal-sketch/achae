...
export default function App() {
  ...
  const handleAddPlataforma = async (nome: string): Promise<boolean> => {
    return await addPlataforma(nome)
  }
    ...
    <div className="app">
      <header className="header">
        <div className="header-inner">
          ...
          <nav className="nav">
            <button
              className={`nav-btn ${activeTab === 'lancamentos' ? 'active' : ''}`}
            ...
            <button
              className={`nav-btn ${activeTab === 'plataformas' ? 'active' : ''}`}
            ...
            <button
              className={`nav-btn ${activeTab === 'resumo' ? 'active' : ''}`}
      ...
      <main className="main">
        {activeTab === 'lancamentos' && (
          <TabLancamentos
            ...
            lancamentosByMesAno={lancamentosByMesAno}
            onAddLancamento={addLancamento}
            onDeleteLancamento={deleteLancamento}
            onDeleteMes={deleteAllLancamentosByMesAno}
          />
        )}
        ...
        {activeTab === 'plataformas' && (
          <TabPlataformas
            ...
            onDeletePlataforma={deletePlataforma}
            getLancamentosByPlataforma={getLancamentosByPlataforma}
          />
        )}
      ...
      <footer className="footer">
        Achaê Promos © 2026 · Controle de Comissões
      </footer>
    </div>
  )
}
