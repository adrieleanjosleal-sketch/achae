...
interface Props {
  ...
  onDeleteMes: (mes: number, ano: number) => void
}
...
export default function TabLancamentos({ plataformas, lancamentosByMesAno, onAddLancamento, onDeleteLancamento, onDeleteMes }: Props) {
  ...
  const [confirmDelete, setConfirmDelete] = useState<{ type: 'lancamento'; id: string; label: string } | { type: 'mes'; mes: number; ano: number; label: string } | null>(null)
  ...
  const confirmAndDelete = () => {
    ...
    if (confirmDelete.type === 'lancamento') onDeleteLancamento(confirmDelete.id)
    else onDeleteMes(confirmDelete.mes, confirmDelete.ano)
                  ...
                  <table className="data-table">
                          ...
                          <tr key={l.id}>
                            ...
                            <td>
                              <button
                                ...
                                onClick={() => setConfirmDelete({ type: 'lancamento', id: l.id, label: `${l.plataforma} — ${mesAnoLabel(mesK, anoK)}` })}
                              >
                                <IconTrash />
                              </button>
                            </td>
                    ...
                    <tfoot>
                      <tr className="total-row">
                        ...
                        <td>
                          <button
                            className="btn-row-delete btn-row-delete-white"
                            title="Excluir todos os lançamentos deste mês"
                            onClick={() => setConfirmDelete({ type: 'mes', mes: mesK, ano: anoK, label: mesAnoLabel(mesK, anoK) })}
                          >
                            <IconTrash />
                          </button>
                        </td>
        ...
        <div className="modal-overlay" onClick={() => setConfirmDelete(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <p className="modal-body">
              {confirmDelete.type === 'lancamento'
                ? <>Deseja excluir o lançamento <strong>{confirmDelete.label}</strong>?</>
                : <>Deseja excluir <strong>todos os lançamentos</strong> de <strong>{confirmDelete.label}</strong>? Essa ação não pode ser desfeita.</>
              }
            </p>
            <div className="modal-actions">
              <button className="btn-cancel" onClick={() => setConfirmDelete(null)}>Cancelar</button>
              <button className="btn-danger" onClick={confirmAndDelete}>Excluir</button>
            </div>
          </div>
        </div>
      )}
