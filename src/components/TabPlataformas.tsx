...
interface Props {
  ...
  onAddPlataforma: (nome: string) => Promise<boolean>
  onDeletePlataforma: (id: string) => void | Promise<void>
  getLancamentosByPlataforma: (nome: string) => Lancamento[]
}

export default function TabPlataformas({
  plataformas,
  onAddPlataforma,
  onDeletePlataforma,
  ...
  const handleAdd = async () => {
    ...
    if (!novaNome.trim()) {
      setErro('Digite o nome da plataforma.')
      return
    }
    ...
    try {
      const ok = await onAddPlataforma(novaNome)

      if (!ok) {
        setErro('Plataforma já cadastrada.')
        return
      }
      ...
      console.error(error)
      setErro('Erro ao adicionar plataforma.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSelect = (nome: string) => {
    setSelected(s => (s === nome ? null : nome))
  }
    ...
    <div className="tab-content">
      <div className="tab-header">
        ...
        <p className="subtitle">
          Gerencie as plataformas de afiliação e consulte o histórico de cada uma.
        </p>
      </div>
      ...
      <div className="card">
        <div className="card-header">
          ...
          <div className="card-subtitle">
            Adicione uma nova fonte de renda afiliada.
          </div>
        </div>
        ...
        <div className="card-body">
          ...
          {erro && (
            <p className="erro-msg" style={{ marginTop: 10 }}>
              {erro}
            </p>
          )}
        </div>
      ...
      <div className="pill-grid">
        {plataformas.map(p => (
          <div
            key={p.id}
            className={`pill-item ${selected === p.nome ? 'selected' : ''}`}
          >
            <button
              className="pill-btn"
            ...
            <button
              className="pill-del"
              title="Excluir"
              onClick={() => onDeletePlataforma(p.id)}
            >
              ✕
            </button>
          </div>
        ...
        <div className="history-card">
          ...
          {lancamentos.length === 0 ? (
            <p className="empty-state">
              Nenhum lançamento para esta plataforma.
            </p>
          ) : (
              ...
              <tbody>
                {lancamentos.map(l => (
                  <tr key={l.id}>
                    ...
                    <td className="td-right td-bold">
                      {formatCurrency(l.valorAdriele + l.valorLarissa)}
                    </td>
                  </tr>
                ))}
              </tbody>
